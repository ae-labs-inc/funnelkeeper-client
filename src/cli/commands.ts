...
export const commands: CommandDef[] = [
  ...
  },
  {
    ...
    flags: [
      { flag: "--product", type: "string", description: "product slug", required: true },
  ...
  {
    name: "product create",
    summary: "create a product in your account",
    flags: [
      ...
      { flag: "--domain", type: "string", description: "product domain" },
    ],
  ...
  },
  {
    name: "detect",
    args: ["url"],
    summary: "auto-detect active analytics, pixels, tag managers, and tools on a website or product",
    flags: [
      { flag: "--product", type: "string", description: "product slug (uses product domain if url omitted)" },
  ...
  },
  {
    name: "connect",
    args: ["source"],
    summary: "connect ga4 | gtm | google-ads | gsc (browser/SA), or semrush | clarity | posthog | opinly | bing | ahrefs (api key)",
    flags: [{ flag: "--product", type: "string", description: "product slug", required: true }],
  ...
  },
  {
    name: "connect sync",
    args: ["source"],
    summary: "sync mysql | postgres | mongo | ga4 | posthog | opinly | gsc | bing now (backfills 30 days, then a year, then everything)",
    flags: [{ flag: "--product", type: "string", description: "product slug", required: true }],
  ...
  { name: "status", summary: "pipeline health: jobs, sources, queue backlog", flags: [], examples: ["fk status"] },
  {
    ...
    flags: [
      ...
      { flag: "--currency", type: "string", description: "iso currency (defaults to the product)" },
    ],
  ...
  { name: "portfolio", summary: "one row per product: spend, revenue, cac, ltv:cac", flags: [], examples: ["fk portfolio"] },
